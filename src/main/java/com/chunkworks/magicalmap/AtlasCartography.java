...
public final class AtlasCartography {
    ...
    /** requires: table inputs; effects: computes output without changing inputs; throws: none. */
    public static ItemStack result(ItemStack first, ItemStack second) {
        try {
            ...
            if (first.is(MagicalMap.ATLAS.get())) {
                if (second.is(Items.FILLED_MAP)) return AtlasPages.add(first, second);
     ...
     * remaining atlas and damages shears; creative shears retain durability; throws: none.
     */
